...
// ARC COORD POS
void arc_validate_head(List *arc_coord_pos_list)
{
  size_t n = arc_coord_pos_list->size;

  /* 1. Build an array of end keys: {end_pos, end_x, end_y} */
  double (*end_keys)[3] = (double (*)[3])malloc(n * sizeof(*end_keys));
  if (!end_keys) return;

  for (size_t i = 0; i < n; i++)
  {
    ArcCoordPos *coordpos = (ArcCoordPos *)list_get(arc_coord_pos_list, i);
    end_keys[i][0] = coordpos->end_fp;
    end_keys[i][1] = coordpos->x2;
    end_keys[i][2] = coordpos->y2;
  }

  /* 2. Sort the end keys */
  qsort(end_keys, n, sizeof(*end_keys), cmp3);

  /* 3. For each item, binary-search its start key in the end keys */
  for (size_t i = 0; i < n; i++)
  {
    ArcCoordPos *coordpos = (ArcCoordPos *)list_get(arc_coord_pos_list, i);
    double start_key[3] = { coordpos->start_fp, coordpos->x1, coordpos->y1 };

    double *found = (double *)bsearch( start_key, end_keys, n, sizeof(*end_keys), cmp3 );
    if (found) {
      /* 4. Exclude self-match: if this item's own end key
       *    equals its start key, check that another copy exists */
      double own_end[3] = { coordpos->end_fp, coordpos->x2, coordpos->y2 };
      if (cmp3(start_key, own_end) == 0)
      {
        /* Look at the neighbors of the found entry.
         * If a duplicate exists before or after, it's a real match. */
        bool real_match = false;

        /* Check entry before 'found' */
        if (found != end_keys[0] && cmp3(start_key, found - 3) == 0)
          real_match = true;

        /* Check entry after 'found' */
        if (!real_match && found != end_keys[n - 1] && cmp3(start_key, found + 3) == 0)
          real_match = true;

        coordpos->arc->is_head = !real_match;
      } else coordpos->arc->is_head = false;
    } else coordpos->arc->is_head = true;
  }
