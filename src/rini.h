...
RINIAPI float rini_get_float_fallback(rini_data data, const char *key, float fallback);
...
RINIAPI int rini_set_float(rini_data *data, const char *key, float value, const char *desc);
...
static float rini_text_to_float(const char *text);
...
// like `rini_get_value_fallback` but for float
float rini_get_float_fallback(rini_data data, const char *key, float fallback)
{
    ...
    for (unsigned int i = 0; i < data.count; i++)
    {
        ...
        {
            // TODO: Detect if conversion fails...
            value = rini_text_to_float(data.entries[i].text);
            break;
        }
    }
...
int rini_set_float(rini_data *data, const char *key, float value, const char *desc)
{
    char text[64];
    snprintf(text, sizeof(text), "%.3f", value);
    return rini_set_value_text(data, key, text, desc);
}
...
static float rini_text_to_float(const char *text)
{
    ...
    if (text[i] == '.')
    {
        ...
        {
            fraction += (float)(text[i] - '0')/divisor;
            divisor *= 10.0f;
        }
