...
void test_fp_to_z(void){
  ...
  // --- Normal case ---
  double result = floor_position_to_z(target_fp, base_bpm, scroll_speed);
  TEST_CHECK(fabs(result - (-100.0f)) < 1e-6);
  TEST_MSG("normal: expected -100, got %f", result);

  // --- Below minimum scroll speed -> clamped to MINIMUM_SCROLL_SPEED ---
  float scroll_speed_below_min = MINIMUM_SCROLL_SPEED - 1.0f;
  double expected_below_min = floor_position_to_z(target_fp, base_bpm, MINIMUM_SCROLL_SPEED);
  double produced_below_min = floor_position_to_z(target_fp, base_bpm, scroll_speed_below_min);
  TEST_CHECK(fabs(expected_below_min - produced_below_min) < 1e-6);
  TEST_MSG("below min: expected %f (clamped), got %f", expected_below_min, produced_below_min);

  // --- Above maximum scroll speed -> clamped to MAXIMUM_SCROLL_SPEED ---
  float scroll_speed_above_max = MAXIMUM_SCROLL_SPEED + 1.0f;
  double expected_above_max = floor_position_to_z(target_fp, base_bpm, MAXIMUM_SCROLL_SPEED);
  double produced_above_max = floor_position_to_z(target_fp, base_bpm, scroll_speed_above_max);
  TEST_CHECK(fabs(expected_above_max - produced_above_max) < 1e-6);
  TEST_MSG("above max: expected %f (clamped), got %f", expected_above_max, produced_above_max);
}
...
void test_zfp_round_trip(void){
  ...
  double z_back = floor_position_to_z(test_fp, base_bpm, scroll_speed);
  TEST_CHECK(fabs(z_back - test_z) < 1e-6);
  TEST_MSG("round-trip z=-100: expected %f, got %f", test_z, z_back);

  // z = -250 -> fp->z should return -250
  double test_z_1 = -250.0f;
  ...
  double z_back_1 = floor_position_to_z(test_fp_1, base_bpm, scroll_speed);
  TEST_CHECK(fabs(z_back_1 - test_z_1) < 1e-6);
  TEST_MSG("round-trip z=-250: expected %f, got %f", test_z_1, z_back_1);

  // z = 150 (positive) -> fp->z should return 150
  double test_z_2 = 150.0f;
  ...
  double z_back_2 = floor_position_to_z(test_fp_2, base_bpm, scroll_speed);
  TEST_CHECK(fabs(z_back_2 - test_z_2) < 1e-6);
...
void test_simd_fp_to_z(void)
{
  ...
  for (int i = 0; i < 6; i++)
  {
    TEST_CHECK(fabs(simd_z_out[i] - normal_z_out[i]) < 1e-6);
